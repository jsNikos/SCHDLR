define(['stateChange/StateChangeView', 'q', 'underscore'], function(StateChangeView, q){
	...
	function StateChangeController(args){
		...
		this.handleStateChangeClick = function(event){			
			...
			if(action === 'POST'){
				requestPostDialogInfo(scope.webSchedulerView.schedulerTableCtrl.week)
		...
		function changeScheduleState($button, action){
			scope.webSchedulerController.findSchedulerTableController().then(function(instanceHolder){
				return requestChangeScheduleState({
					action : action,
